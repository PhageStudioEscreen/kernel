...
static int check_fw_chip_ver(dev_data * dev_entry, xchange_data * xdata)
{
	...
	skb = rtk_hci_cmd_sync(xdata->dev_entry->udev, opcode, 0, NULL, 1000);
	if (IS_ERR(skb) || !skb) {
...
static int rtk_enhanced_download_mode_enable(xchange_data *xdata)
{
	...
	skb = rtk_hci_cmd_sync(xdata->dev_entry->udev, opcode, 1, &val, 1000);
	if (IS_ERR(skb) || !skb) {
		RTKBT_ERR("send %04x error (%lu)", opcode, PTR_ERR(skb));
		return -EIO;
	}
	if (skb->len != sizeof(*ev)) {
		RTKBT_ERR("got invalid cmd complete, %u %u", skb->len,
				sizeof(*ev));
		goto err;
	}
...
static int download_patch_sections(dev_data *dev_entry, xchange_data *xdata)
{
	...
	list_for_each_entry_safe(sec, tmp, &list_secs, list) {
		...
		i = 0x80;
		skb = rtk_hci_cmd_sync(xdata->dev_entry->udev, DOWNLOAD_OPCODE,
				       1, &i, 200);
		if (IS_ERR(skb) || !skb) {
			ret_val = -EIO;
...
static int btrtl_finalize_download(dev_data *dev_entry)
{
	...
	skb = rtk_hci_cmd_sync(dev_entry->udev, opcode, len,
			       params, 1000);
	if (IS_ERR(skb) || !skb) {
...
static int rtl_wait_fw_ready_evt(xchange_data *xdata)
{
	...
	while (count-- > 0) {
		ret = usb_interrupt_msg(xdata->dev_entry->udev,
					xdata->pipe_in,
...
static int rtl_download_firmware_v3(dev_data * dev_entry, xchange_data *xdata)
{
	...
	if (dev_entry->fw_type == FW_TYPE_V3_2) {
		skb = rtk_hci_cmd_sync(dev_entry->udev, HCI_VENDOR_WRITE_TB, 0,
				       NULL, 1000);
		if (IS_ERR(skb) || !skb) {
				   ...
				   int fw_size)
{
	...
	skb = rtk_hci_cmd_sync(dev_entry->udev, cmd_opc, 0, NULL, 1000);
	if (IS_ERR(skb) || !skb) {
	...
	for (i = 0; i < num_sections; i++) {
		RTKBT_DBG("rtl section: %u", sizeof(*section));
...
get_ver:
	...
	list_for_each_safe(pos, next, &list_secs) {
		...
		if (sec->image_id == IMAGE_ID_F000 ||
			...
			skb = rtk_hci_cmd_sync(dev_entry->udev, opcode, 4,
					       payload, 200);
			if (IS_ERR(skb) || !skb) {
			...
			skb = rtk_hci_cmd_sync(dev_entry->udev, opcode, 0, NULL,
					       200);
			if (IS_ERR(skb) || !skb) {
				RTKBT_ERR("cmd (%04x) failed, %ld", opcode,
					  PTR_ERR(skb));
...
static int download_acldata(xchange_data *xdata)
{
	...
	buf = kmalloc(sizeof(*hdr) + 1 + max_payload_len, GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
	...
	for (i = 0; i < frag_num; i++) {
		...
		ret = usb_bulk_msg(udev, pipe_bulk_out, buf, pkt_len,
					&len, 3000);
		...
		while (--loop > 0) {
			...
			ev = (struct hci_ev_patch_status_evt *)evt;
			if (ev->evt != 0xff)
				continue;
			if (len < sizeof(*ev))
				continue;
...
struct sk_buff *rtk_hci_cmd_sync(struct usb_device *udev, u16 opcode, u8 plen,
				 u8 *param, u32 timeout)
{
	...
	util_hexdump(xdata->send_pkt, xdata->pkt_len);
	ret = usb_control_msg(udev, xdata->pipe_out, 0, USB_TYPE_CLASS, 0, 0,
			      (void *)xdata->send_pkt, xdata->pkt_len, 1000);
	if (ret < 0) {
	...
	while (loop-- > 0) {
		...
		ret = usb_interrupt_msg(udev, xdata->pipe_in,
					(void *)xdata->rcv_pkt,
					PKT_LEN, &rp_len, timeout);
		if (ret < 0) {
			RTKBT_ERR("%s; No usb intr for hci event, err %d",
				  __func__, ret);
			break;
		}

		/* RTKBT_DBG("opcode %04x, ret %d, rp_len %d",
		 * 		xdata->cmd_hdr->opcode, ret, rp_len);
		 * if (rp_len > 0)
		 * 	util_hexdump(xdata->rcv_pkt, rp_len);
		 */

		if (CMD_CMP_EVT == xdata->evt_hdr->evt &&
		    xdata->cmd_hdr->opcode == xdata->cmd_cmp->opcode) {
			match = true;
			break;
		}
	}
